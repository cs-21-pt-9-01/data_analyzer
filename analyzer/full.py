...
def full_run(_dir: str, plot_type: str):
    for _d in os.listdir(_dir):
        ...
        for r in res:
            ...
            if plot_type == 'plot':
                plot(plot_type, power_j_total, x, f'{path}/power_j_total.png', 'Benchmark no.', 'Power (Joules)',
                     r.name.split('/')[-1])
            elif plot_type == 'bar':
                bar({zone: np.average(value) for zone, value in power_j_total.items()},
                    f'{path}/power_j_overall_avg', f'Power (Joules, avg - {len(r.run_metrics)} runs)', 'RAPL Zone',
                    r.name.split('/')[-1])
            elif plot_type == 'curve':
                plot(plot_type, power_j_total, [], f'{path}/watts_since_last_curve.png', 'Time (seconds)',
                     'Power (Watts) since previous measurement', r.name.split('/')[-1])
